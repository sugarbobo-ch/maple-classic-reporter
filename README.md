# 新楓之谷：經典版《自動外掛檢舉工具》 v2.4.0 (MapleStory Classic Auto Reporter)
...
本版聚焦多人批次檢舉、可恢復送件流程與共用證據安全清理：

- 可一次建立多人檢舉名單，逐人保存送件紀錄，並為個別角色設定不同說明。
- 同一批檢舉只上傳一次證據；自動模式依序送出，手動模式提供逐人填表助手與進度提示。
- 送件中斷或結果不明時會保留批次進度，可確認已完成或允許重試，避免重複建立批次。
- 清理或刪除紀錄時會辨識共用的本機與 Google Drive 證據，避免留下孤兒檔案或誤刪仍被引用的證據。
- 改善錄影、循環回放、設定與歷史紀錄的狀態提示，並補上批次流程、證據生命週期與表單互動測試。

完整內容請參閱：[v2.4.0 發布說明](docs/releases/v2.4.0.md)。
...
4. **手動或自動檢舉**：每次送件前可選擇檢舉方式。手動模式會先上傳證據，再提供依官方表單順序排列的欄位與複製按鈕；自動模式則使用 Playwright 填寫官方表單，並可選擇背景完成或顯示瀏覽器過程。
...
10. **首次啟動設定助理**：第一次使用時會介紹蒐證、辨識、上傳、檢舉與回顧流程，並依序完成錄影、OCR、雲端與檢舉方式設定；也可選擇只試用蒐證與辨識、不連接或上傳到任何雲端。之後可從設定重新觀看。
...
Compress-Archive `
  ...
  -DestinationPath .\dist\MapleClassicReporter-v2.4.0-windows-x64.zip `
...
1. 在「檢舉證據回報表單」建立角色名單：輸入半形／全形空白、逗號或按 Enter 即可加入名字，也支援貼上時自動分隔。每個名字的 × 可刪除該人，輸入區右側 × 可全部刪除。開啟角色 OCR 自動填入時，會先加入一名候選角色，其餘可複選；請確認辨識結果是否正確。
2. 在「上傳與帳號」選擇 **「優先上傳目的地」**：Google Drive 適合長期保存供官方審查；Discord 適合 10 MiB 內的短片。使用 Google Drive 時可按 **「前往雲端資料夾」** 查看檔案。
3. 確認共用的伺服器、地圖與說明；需要時可展開個別角色修改說明。按 **「開始檢舉 N 人」** 後，影片只上傳一次，自動模式依序逐人送出，每人分開保存紀錄及追蹤處分。手動模式則逐人提供填表資料，確認完成後切換下一人。
4. 上傳失敗時不會送出官方表單。請檢查帳號或 Discord 頻道連結，也可以切換優先上傳目的地後重試。
5. 任一人的送件失敗會暫停整批。若顯示「結果待確認」，請確認官方是否已收到，再選擇「確認已完成」或「允許重試」；繼續時會跳過已完成的人。可從任一待辦紀錄繼續同批檢舉，草稿會保留名單及個別說明。
6. 啟用自動刪除時，會等整批全部確認完成才清理本機影片；刪除個別紀錄時，仍被其他紀錄引用的共用證據會保留。
