# 新楓之谷：經典版《自動外掛檢舉工具》 v1.1.2 (MapleStory Classic Auto Reporter)
...
相較於上一版 `v1.1.1`，本版重點如下：

- 修正地圖 OCR 自動填入：支援只辨識到單行地圖名稱、較小的小地圖標題間距，以及 OCR 執行緒完成後的結果補填；降低將區域名稱誤判成地圖名稱的機率。
- 回放緩衝與本機事證共用更穩定的地圖名稱校正與多影格投票流程，提升像「東方岩石山」等地圖的辨識結果。
- 上傳成功並確認官方表單送出後自動關閉預覽視窗；移除預覽頁的「點擊前往查看」按鈕，保留連結欄位與歷史紀錄檢視。
- 「選擇本機事證」預設開啟 `%LOCALAPPDATA%\\MapleClassicReporter\\recordings`，直接找到程式產生的錄影檔。
- 只有在確認表單送出成功後才關閉表單瀏覽器，失敗時保留畫面方便檢查與重試。
...
Compress-Archive `
  ...
  -DestinationPath .\dist\MapleClassicReporter-v1.1.2-windows-x64.zip `
